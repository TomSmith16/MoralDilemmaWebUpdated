/////////////////////ERROR IN FIREBASE, SAVECHOICE NOT IN RIGHT PLACE FOR FINAL OF FIRST SECTION, MISSES OUT LAST ONE. MESS AROUND WITH IT TO SEE IF WE CAN FIND WHERE IT NEEDS TO GO. USE CONSOLE TO TRACK.
...
var gamedilemmas = [0, 1, 6, 7, 14, 15, 16, 17, 18, 19];
var realdilemmas = [2, 3, 4, 5,  8,  9, 10, 11, 12, 13];
...
var gamesurveyfinished = false;
var realsurveyfinished = false;
var fingerprint = "";
var aleftoffset = "";
var bleftoffset = "";
var atopoffset = "";
...
var gamedilemmasnumber = -2;
var realdilemmasnumber = -2; 
var dilemmascounter = -2;
...
function nextDilemma()
{
	...
	if(mfqrandom == 1)
	{
		//////////// CHECK FOR DILEMMAS LENGTH. ONCE THE GAME DILEMMAS HAVE BEEN CYCLED THROUGH THE SURVEY FINISHES. FIND WHERE THIS IS. look around survey finished & dilemmas.length, as well as where #thanks loads from
		if(gamesurveyfinished == false)
		{
			
			gamedilemmasnumber = gamedilemmasnumber + 2;
			i = gamedilemmas[gamedilemmasnumber];
			console.log("GI: " + gamedilemmasnumber);
			
			if(gamedilemmasnumber >= gamedilemmas.length-2)  //Finished all the games dilemmas, is greater or equal to the length of gamesdilemmas array
			{
				surveyfinished = true;
				console.log("should end here");
				
				/*
				fade out survey fade in mfq, switch mfq random number.
				load MFQ survey for the next section.
				mfqrandom = opposite of current
				*/
			}
		}
		else
		{
			realdilemmasnumber = realdilemmasnumber + 2;
			i = realdilemmas[realdilemmasnumber];
			console.log("RWI: " + realdilemmasnumber);
			
			if(realdilemmasnumber >= realdilemmas.length-2)  //Finished all the real dilemmas, is greater or equal to the length of realdilemmas array
			{
				realsurveyfinished = true;
				surveyfinished = true;
				console.log("should end here");
				
				/*
				fade out survey fade in mfq, switch mfq random number.
				load MFQ survey for the next section.		Change the MFQ confirm buttons to check 
				mfqrandom = opposite of current
				*/
			}
			
			//do the real dilemmas
		}
	}
	...
	if(mfqrandom = gamescenario/0/whatever)
	{
		gamedilemmasnumber = 0 declare at the beginning
		i = gamedilemmas[gamedilemmasnumber]
		gamedilemmasnumber = gamedilemmasnumber + 2;
		if(gamedilemmasnumber is greater or equal to the length of gamesdilemmas array)  Finished all the games dilemmas
		{
			load MFQ survey for the next section.
			mfqrandom = opposite of current
		}
	...
	{
		realdilemmasnumber = 0 declare at the beginning
		i = realdilemmas[realdilemmasnumber]
		realdilemmasnumber = realdilemmasnumber + 2;
		if(realdilemmasnumber is greater or equal to the length of realdilemmas array)  Finished all the real dilemmas
		{
			load MFQ survey for the next section.		Change the MFQ confirm buttons to check 
			mfqrandom = opposite of current
		}
	...
	*/
	...
	{	
		...
		$("#survey").css("background-color", gamebackground);

		$("body").toggleClass("transparent");
		document.getElementById("object").innerHTML = "Video Game Moral Dilemma";
	}
	else
	{	
		//document.body.style.backgroundImage = "url(bg.png)";
		//document.body.style.backgroundsize = "cover";
		$("#survey").css("background-color", realbackground);

		$("body").toggleClass("transparent");
		document.getElementById("object").innerHTML = "Real World Moral Dilemma";
	}
		 ...
		 if(i == 8)
			{
				if(fatmanvar == 0)
				{
					charhovertext1 = "Large man (Stranger) = Dead";
...
function characterSetUp()
{
	...
	switch(i)
	{
		...
		{
			//Info
			document.getElementById("dilemmainfo1").innerHTML += "The train is headed towards the people on the track, pushing the large man will stop the train. \nWhat would <b>you</b> do?"
			//Limit charamount to 1,
			charamount = 0;
			//charamount1 to 3-5
		...
		case 18:
		{
			document.getElementById("dilemmainfo1").innerHTML += "A horde of zombies is about to catch your party, you can push the individual into the horde to give your party time to escape. What would <b>you</b> do? "
			charamount = 0;
			//charamount1 to 3-5
			charamount1 = Math.floor((Math.random() * 3)+3);
...
function saveChoice()
{
	...
	if(mfqrandom == 1)
	{
		...
		{
			var responseObj = {
					dilemma: dilemmas[gamedilemmas[gamedilemmasnumber]],
					characterL: [],
					characterR: [],
					outcome: selected,
		...
		{
		var responseObj = {
					dilemma: dilemmas[realdilemmas[realdilemmasnumber]],
					characterL: [],
					characterR: [],
					outcome: selected,
					/*Could save character chosen to die rather than saved
...
{
	//PARTICIPANTS INFO, MOVE THIS TO ITS OWN SECTION IF DOESNT WORK AFTER CHANGES
	var checkedValue = []; 
	var checkednumber = 0;
	...
	$("#confirm").click(function(){
		...
		/*If survey is not over, load new dilemma*/
			if(surveyfinished == false)
			{
				...
				if(gamedilemmasnumber < gamedilemmas.length)
				{  
			...
			else
				{
					...
					if(gamesurveyfinished && realsurveyfinished)
					{
					...
					{
						//saveChoice();
						//alert("You have finished half of the survey, almost there!");
						//surveyfinished = true;
						...
						$("#survey").fadeOut(500, function() {
							...
							if(mfqrandom == 1)
								{	
									alert("You have finished section 1 of 2 about video games, you are now starting section 2 on the real world!");
									gamesurveyfinished = true;
								...
								{
									alert("You have finished section 1 of 2 about the real world, you are now starting section 2 about video games!");
									realsurveyfinished = true;
									surveyfinished = false;
									document.body.scrollTop = 0; // For Safari
									document.documentElement.scrollTop = 0; // For Chrome, Firefox, IE and Opera
					...
					*/
					...
					else
					{
						...
						else
						{
							...
							realsurveyfinished = true;
							Fade in survey
							Refresh gamesdilemmasnumber
							Switch to other dilemmas array 
							
							*/
							realdilemmasnumber = -2;
...
{
	//MFQ Randomiser
	mfqrandom = Math.floor((Math.random() * 2));			//0 = Realworld 1 = Games
	...
	dilemmab.src = dilemmas[dilemma1];
		...
		document.getElementById("dilemmainfo").innerHTML += "The train is headed towards three people, pulling the lever will switch the track. What would <b>you</b> do?"
